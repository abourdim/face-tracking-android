#!/usr/bin/env bash
# ============================================================================
#  Face Tracking Kids Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/app-face-tracking}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.facetracking}"
AVD_NAME="${AVD_NAME:-face_tracking_test}"
...
cmd_test() {
  ...
  local out="/tmp/face-tracking-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Face Tracking Kids Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
